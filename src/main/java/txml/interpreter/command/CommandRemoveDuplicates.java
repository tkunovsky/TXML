...
public class CommandRemoveDuplicates implements Command {
    ...
    public void execute(InstructionsInterpreter interpreter, String operand1, String operand2, String operand3, String result) {
        ...
        List<TNode> oldList = new ArrayList<>();
        
        for (int i = 0; i < tNodeListOld.getLength(); i++) {
            oldList.add(tNodeListOld.item(i));
        }

        TNodeList tNodeListNew = new TNodeList(interpreter.getLastSettings(), new ArrayList<>(new HashSet<>(oldList)));
